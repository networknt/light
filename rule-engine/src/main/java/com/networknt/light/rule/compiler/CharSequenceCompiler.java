...
public class CharSequenceCompiler<T> {
	private static final org.slf4j.Logger logger = LoggerFactory.getLogger(CharSequenceCompiler.class);
	...
	public CharSequenceCompiler(ClassLoader loader, Iterable<String> options) {
		...
		//final JavaFileManager fileManager = compiler.getStandardFileManager(
		...
		final JavaFileManager fileManager = new CustomClassloaderJavaFileManager(
				loader, standardJavaFileManager);
	...
	public synchronized Map<String, Class<T>> compile(
		...
		try {
			...
			throw new CharSequenceCompilerException(classes.keySet(), e,
					diagnostics);
    ...
    public synchronized void compile(
            final Map<String, String> classes, Map<String, Rule> ruleCache)
            throws CharSequenceCompilerException {
        List<JavaFileObject> sources = new ArrayList<JavaFileObject>();
        ...
        try {
            // For each class name in the compile map, get its compiled class and put it in the ruleCache
            for (String qualifiedClassName : classes.keySet()) {
                Class<Rule> newClass = (Class<Rule>)loadClass(qualifiedClassName);
                // test if it is abstract
                if(!Modifier.isAbstract(newClass.getModifiers())) {
                    Rule rule = newClass.newInstance();
                    ruleCache.put(qualifiedClassName, rule);
                }
            }
        } catch (ClassNotFoundException e) {
            logger.error("Exception:", e);
...
final class FileManagerImpl extends ForwardingJavaFileManager<JavaFileManager> {
	...
	private URI uri(Location location, String packageName, String relativeName) {
		return CharSequenceCompiler.toURI(location.getName() + '/'
...
final class JavaFileObjectImpl extends SimpleJavaFileObject {
	...
	JavaFileObjectImpl(final String baseName, final CharSequence source) {
		super(CharSequenceCompiler.toURI(baseName
				+ CharSequenceCompiler.JAVA_EXTENSION), Kind.SOURCE);
		this.source = source;
	}
...
final class ClassLoaderImpl extends ClassLoader {
	private static final org.slf4j.Logger logger = LoggerFactory.getLogger(ClassLoaderImpl.class);
